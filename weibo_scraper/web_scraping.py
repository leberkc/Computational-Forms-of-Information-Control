   ...
   while not queue.empty():
       ...
       for i in range(len(data["content"])):
           if data["url"][i] not in exisiting_urls:
               try:
                   cur.execute('''INSERT INTO {} (Post_Id, Content, url, uid, pid, mid, pubdate, tested, test_date, status)
                                   VALUES (NULL, %s,%s, %s, %s, %s, CURDATE(), DEFAULT, DEFAULT, DEFAULT)'''.format(table), \
